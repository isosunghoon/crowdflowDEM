...
def update(t):
    ...
    for human in humans:
        ...
        #save human movement
        if t%10==0:
            with open(f"./res/{sys.argv[0].split('.')[0]}_{''.join(sys.argv[1].split('.')[:-1]).split('/')[-1]}.txt", "a") as file:
                file.write(f"{human.realid} {t//10} {human.pos[0]} {human.pos[1]}\n")
...
simulation = {'eye':info['eye'],'alpha':info['alpha'], 'beta':info['beta'], 'will_coef':info['will_coef'], 'stiffness':info['stiffness'], 'restitution':info['restitution'], #hyperparameter
              'frame':5100, 'dt':0.004, 'w': 1200, 'h': 738, 'radius':10, 'mass':500, 'speed_mean':info['speed_mean'], 'speed_var':info['speed_var'], 'padding':10, #environment variable
              'human_rate':10,'will_rate':info['will_rate'], 'same_penalty':info['same_penalty'], 'obstacle_advantage':info['obstacle_advantage'],'angle':info['angle'],'scam_radius':info['s_radius'],'min_arg':info['min_arg'], #modification
              'df':[],'humancount':0
            }
debug_var = time.time()
simulation['df'] = pd.read_csv('./processed_tracks.csv')
simulation['df'].sort_values(by='Starting Frame', inplace=True)
simulation['humancount'] = 0

with open(f"./res/{sys.argv[0].split('.')[0]}_{''.join(sys.argv[1].split('.')[:-1]).split('/')[-1]}.txt", "w") as file:
    pass
