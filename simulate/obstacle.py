...
def add_human(t):
    if t <= 1000:
        ...
        x_range_min = 50 + 100*((t%(3*simulation['human_rate']))//simulation['human_rate']) + simulation['radius']
        x_range_max = x_range_min + 100 - simulation['radius']*2
        if t % simulation['human_rate'] == 0:
            h = Human(pos = (np.random.randint(x_range_min,x_range_max), 0), direction = (0,1), speed = s, color = '#179BAE', radius = simulation['radius'], mass = simulation['mass'])
            humans.append(h)
        if t % simulation['human_rate'] == 5:
            s = np.random.normal(loc=simulation['speed_mean'], scale=simulation['speed_var'])
            h = Human(pos = (np.random.randint(x_range_min,x_range_max), 900), direction = (0,-1), speed = s, color = '#4158A6', radius = simulation['radius'], mass = simulation['mass'])
            humans.append(h)
...
simulation = {'eye':info['eye'],'alpha':info['alpha'], 'beta':info['beta'], 'will_coef':info['will_coef'], 'stiffness':info['stiffness'], 'restitution':info['restitution'], #hyperparameter
              'frame':5000, 'dt':0.004, 'w': 400, 'h': 900, 'radius':10, 'mass':500, 'speed_mean':info['speed_mean'], 'speed_var':info['speed_var'], 'padding':10, #environment variable
              'human_rate':20,'will_rate':info['will_rate'], 'same_penalty':info['same_penalty'], 'obstacle_advantage':info['obstacle_advantage'],'angle':info['angle'],'s_radius':info['s_radius'],'min_arg':info['min_arg'] #modification
            }
debug_var = time.time()
...
obstacles.append(Obstacle([Vec((0,0)),Vec((50,0)),Vec((50,900)),Vec((0,900))]))
obstacles.append(Obstacle([Vec((150,400)),Vec((150,500)),Vec((250,500)),Vec((250,400))]))
obstacles.append(Obstacle([Vec((350,0)),Vec((400,0)),Vec((400,900)),Vec((350,900))]))
