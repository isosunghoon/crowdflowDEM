...
def add_human(t):
    if t == 2:
        s = 100
        h = Human(pos = (200, 0), direction = (0,1), speed = s, color = '#179BAE', radius = simulation['radius'], mass = simulation['mass'])
        humans.append(h)
        h2 = Human(pos = (200, 900), direction = (0,-1), speed = s, color = '#4158A6', radius = simulation['radius'], mass = simulation['mass'])
        humans.append(h2)
...
simulation = {'eye':info['eye'],'alpha':info['alpha'], 'beta':info['beta'], 'will_coef':info['will_coef'], 'stiffness':info['stiffness'], 'restitution':info['restitution'], #hyperparameter
              'frame':2000, 'dt':0.004, 'w': 400, 'h': 900, 'radius':10, 'mass':500, 'speed_mean':info['speed_mean'], 'speed_var':info['speed_var'], 'padding':10, #environment variable
