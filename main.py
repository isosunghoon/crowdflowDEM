...
def init():
    simulation['obstacles'].append(Obstacle([Vec((60,160)),Vec((200,160)),Vec((200,200)),Vec((60,200))]))

    os.makedirs(f'res/{simulation['name']}', exist_ok=True)
    with open(f'./res/{simulation['name']}/init.dem', 'w') as file:
        json.dump(simulation, file, indent=4, cls=CustomEncoder)

def add_human():
    if simulation['t'] == 1:
        h = Human(pos = (100, 100), direction = (0,1), color = '#77E4C8', radius = simulation['radius'], mass = simulation['mass'], speed = 100)
        simulation['humans'].append(h)
        h = Human(pos = (100, 140), direction = (0,1), color = '#478CCF', radius = simulation['radius'], mass = simulation['mass'], speed = 100)
        simulation['humans'].append(h)
...
def human_will():
    def f1(arg, dist):
        return np.sign(arg+0.0001) / (1 + abs(arg)) / dist / dist * simulation['alpha']
    def f2(dist):
...
def save():
    with open(f'./res/{simulation['name']}/{simulation['t']}.dem', 'w') as file:
        json.dump(simulation, file, indent=4, cls=CustomEncoder)
...
simulation = {'humans':[], 'obstacles':[], 'collided_human_pair':[], 'collided_human_obs':[], 't': 0, #variable
              'eye':5, 'alpha':10000, 'beta':0.05, 'will_coef':500, 'stiffness':5e5, 'restitution':0.3, #hyperparameter
              'frame':1000, 'dt':0.005, 'w': 300, 'h': 800, 'radius':10, 'mass':1, 'speed_mean':100, 'speed_var':30, #environment variable
              'name':'temp'}
